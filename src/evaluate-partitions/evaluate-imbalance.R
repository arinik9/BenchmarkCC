...
compute.imbalance.from.membership = function(g, membership, output.type = "count"){
	# see calculation details in page 9 of this PhD thesis: 
	#		J.-H. Lange, « Multicut Optimization Guarantees & Geometry of Lifted Multicuts », PhD thesis, Saarland University, 2020.
    ...
    edge.cut.values <- as.integer(!(membership[edge.mat[,1]] == membership[edge.mat[,2]])) # 0 means two vertices are in the same cluster, otherwise 1
