...
{	
    if(startsWith(algo.name,COR.CLU.ILS) || startsWith(algo.name,COR.CLU.GRASP))
    ...
    else if(startsWith(algo.name,get.ExCC.code(enum.all=FALSE))) {
		result <- read.table(file.path(folder.name, result.filename))$V1
	}
    else if(startsWith(algo.name,get.ExCC.code(enum.all=TRUE))) {
		result <- read.table(file.path(folder.name, result.filename))$V1
	}
		...
		result <- load.membership.from.0.1.vector.multicut(folder.name, result.filename, "\t")
	}
    else if(startsWith(algo.name,COR.CLU.GASP) || startsWith(algo.name,COR.CLU.NIFTY)
            || startsWith(algo.name,COR.CLU.MLMSB) || startsWith(algo.name,COR.CLU.ZONOCC)
            || startsWith(algo.name,COR.CLU.SA.CC) || startsWith(algo.name,COR.CLU.TS.CC)
            || startsWith(algo.name,COR.CLU.Brusco.VNS.CC)
            || startsWith(algo.name,CODE.COR.CLU.SPONGE.sym.CC))
...
load.membership.from.0.1.vector.multicut  <- function(folder.name, result.filename, sep="\t"){
    result = read.table(file.path(folder.name, result.filename), sep=sep)
