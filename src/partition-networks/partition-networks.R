...
load.result.files = function(part.folder, algo.name) {
	...
	if(nb.result.file>0){
		for (i in 1:nb.result.file) {
			...
			mbrshp <-
					load.external.partition(part.folder, algo.result.filename, algo.name, keep.tmp = TRUE)
			mbrshps[[i]] = mbrshp
		}
	}
...
load.membership.files = function(part.folder) {
	...
	if(file.exists(file.path(part.folder,"allResults.txt"))){
...
#if(!file.exists(file.path(part.folder,"log.txt"))){
...
#}
	...
	if ((process.algo && process.mbrshp) || force) {
		...
		if(process.mbrshp){
			...
			# the method that we use already outputs a result file with membership format. 
		}
		...
		keep.algo.log.files, plot.format, plot.layout, force)
{
	...
	tlog(16, "start to partition networks")
		...
		for(algo.name in cor.clu.heur.algos){
			...
			for(rep.no in heur.reps){	
				...
				if (dir.exists(net.folder)) {
					...
					if(startsWith(algo.name,CODE.COR.CLU.SPONGE.sym.CC))
						graph.name = paste0(graph.desc.name, ".graphml")
					
					temp.file = file.path(net.folder, "temp.txt")
					
					# ------------------------------------
					# the following three partitioning methods needs a slightly different graph format
					# workaround: we tackle this here, instead of in the graph generation process
					if(startsWith(algo.name,CODE.COR.CLU.GAEC.KLj.CC) || startsWith(algo.name,CODE.COR.CLU.ICP.GAEC.KLj.CC) || startsWith(algo.name,CODE.COR.CLU.MP.GAEC.KLj.CC) ){
						input.file = file.path(net.folder, graph.name)
		...
		for(algo.name in cor.clu.exact.algos){
			...
			for(rep.no in exact.reps){	
				...
				if (dir.exists(net.folder)) {
					...
					#if(!file.exists(file.path(part.folder,"strengthedModelAfterRootRelaxation.lp")))
					...
					# ------------------------------------
					if(startsWith(algo.name,CODE.COR.CLU.GAEC.KLj.CC) || startsWith(algo.name,CODE.COR.CLU.ICP.GAEC.KLj.CC)){
						if(file.exists(temp.file))
							unlink(temp.file)
					}
					# ------------------------------------
